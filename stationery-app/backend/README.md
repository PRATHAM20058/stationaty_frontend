...
| GET    | `/items`                | All items. |
| POST   | `/items`                | Item minus `id` -> `201` full item. |
| PUT    | `/items/:id`            | Full replace incl. absolute `stockQty`. |
| DELETE | `/items/:id`            | `204`. |
| GET    | `/bills`                | All bills with nested `items`. |
| POST   | `/bills`                | Full bill; server assigns `id` + `billNo`. |
| PUT    | `/bills/:id/payment`    | Payment patch **or** full-bill edit (idempotent upsert). |
| DELETE | `/bills/:id`            | `204`. |
